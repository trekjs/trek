  ...
  _createClass(Trek, {
    ...
    run: {
      value: function run() {
        ...
        // TODO: https
        let app = (_ref = this).listen.apply(_ref, arguments);
        console.log(chalk.green("  * Trek %s application starting in %s on http://%s:%s"), Trek.version, Trek.env, app.address().address === "::" ? "localhost" : app.address().address, app.address().port);
      }
    }
  }, {
    env: {
