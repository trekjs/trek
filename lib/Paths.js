...
const debug = (0, _debug3.default)('trek:paths');
...
class Paths {
   ...
   * @param {String} [formats='toml|json|js|yml'] The formats can parse and
   *                                              render
   */
  constructor(root) {
    let formats = arguments.length <= 1 || arguments[1] === undefined ? 'toml|json|js|yml' : arguments[1];
   ...
   */
  get(key) {
    let absolute = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];

    const value = this.blueprint.get(key) || key;
    const pattern = value.glob || value.with || value;
    const res = (0, _glob2.default)(pattern, { sync: true, realpath: absolute, cwd: this.root });
  ...
  getPattern(key) {
    ...
    const value = this.blueprint.get(key);
