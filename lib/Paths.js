...
const debug = _debug3['default']('trek:paths');
...
var Paths = (function () {
  ...
  function Paths(root) {
    var formats = arguments.length <= 1 || arguments[1] === undefined ? 'toml|json|js|yml' : arguments[1];
  ...
  Paths.prototype.get = function get(key) {
    ...
    var pattern = value.glob || value['with'];
    if (pattern) {
      var res = _glob2['default'](pattern, { sync: true, realpath: absolute, cwd: this.root });
      return value.multi ? res : res[0];
    }
    return _path2['default'].join(absolute ? this.root : '', value);
  };
  ...
  Paths.prototype.getPattern = function getPattern(key) {
    if (!this.blueprint.has(key)) return null;
    var value = this.blueprint.get(key);
