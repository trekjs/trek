...
var _nunjucks2 = require('nunjucks');

var _nunjucks3 = _interopRequireDefault(_nunjucks2);
...
var Config = (function () {
  ...
  Config.prototype.initStores = function initStores() {
    // nunjucks configure
    this.nunjucks = new _nunjucks3['default'].configure({
      autoescape: true
    });
  ...
  Config.prototype.render = function render() {
    var _nunjucks;

    return (_nunjucks = this.nunjucks).render.apply(_nunjucks, arguments);
  };
  ...
  Config.prototype.loadConfigs = function loadConfigs() {
    ...
    forEach(function (item) {
      ...
      try {
        var s = nc || _this.compile(_this.root + '/' + c, n ? t : null, e);
  ...
  Config.prototype.compile = function compile(filename, namespace, env) {
    var context = this.render(filename);
