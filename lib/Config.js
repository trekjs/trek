...
class Config {
  ...
  *loadList() {
    ...
    for (let _ref of this.stores.entries()) {
  ...
  *render(filename) {
    ...
    let content = yield (0, _mzFs.readFile)(filename, 'utf-8');
  ...
  parse(content, filename) {
    let ext = (0, _path.extname)(filename).substring(1);
    // parser
