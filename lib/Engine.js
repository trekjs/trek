...
var Engine = (function (_Koa) {
  ...
  Engine.prototype.run = function run() {
    ...
    return _co2['default'].call(this, function* () {
      ...
      var app = this.listen.apply(this, args);
      this.logger.info(_chalk2['default'].green('%s application starting in %s on http://%s:%s'), Trek.version, Trek.env, app.address().address === '::' ? '127.0.0.1' : app.address().address, app.address().port);
...
_trekRouter2['default'].METHODS.forEach(function (m) {
  ...
  Engine.prototype[m] = eval(`(function ${ _lodashStringCamelCase2['default'](name) }(path, ...handlers) {
