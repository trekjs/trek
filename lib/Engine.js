...
class Engine extends _koa2.default {
  ...
  use(x) {
    return super.use.call(this, (0, _koaConvert2.default)(x));
  }
  ...
  loadRoutes() {
    ...
    try {
      ...
      this.routeMapper.routes.forEach(r => {
        ...
        const path = (0, _path.join)(controllersPath, controller) + '.js';
        ...
        r.verb.forEach(m => {
          ...
          if (c && (a = c[action])) {
            ...
            this[m](r.path, _koaConvert2.default.compose(...a));
          }
        });
      });
    } catch (e) {
  ...
  *bootstrap() {
    ...
    this.use((ctx, next) => {
      ...
      var handler = _ctx$app$router$find2[0];
      var params = _ctx$app$router$find2[1];
  ...
  *loadServices() {
    ...
    let file;
    for (file of files) {
      var name = (0, _path.basename)(file, '.js').replace(/^[0-9]+-/, '');
  ...
  serveFile(path, file, options) {
    ...
    return this.get(path, function* (next) {
      yield (0, _koaStatic2.default)(dir, options).call(this, next);
    });
  }
