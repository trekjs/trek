...
var _basename$dirname$join = require('path');
...
let Engine = (function (_Koa) {
  ...
  Engine.prototype.loadRoutes = function loadRoutes() {
    ...
    try {
      ...
      this.routeMapper.routes.forEach(function (r) {
        ...
        let path = _basename$dirname$join.join(controllersPath, controller) + '.js';
        let c = require(path);
  ...
  Engine.prototype.loadServices = function loadServices() {
    ...
    return _co2['default']((function* () {
      ...
      for (var _iterator = files, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
        ...
        let name = _basename$dirname$join.basename(file, '.js').replace(/^[0-9]+-/, '');
        let service = require(file)(this, this.config);
        if (service) {
          this.setService(name, service);
          this.logger.info(_chalk2['default'].green(`service:${ name } init ...`));
          if (service.promise) yield service.promise;
          this.logger.info(_chalk2['default'].green(`service:${ name } booted`));
        }
      }
    }).bind(this));
  };
  ...
  Engine.prototype.serveFile = function serveFile(path, file, options) {
    file = _basename$dirname$join.dirname(file);
  ...
  Engine.prototype.listen = function listen() {
    ...
    this.use(function* dispatcher(next) {
      ...
      let handler = _app$router$find[0];
      let params = _app$router$find[1];
  ...
  Engine.prototype.run = function run() {
    ...
    return this.loadServices().then(function () {
      ...
      let app = _this3.listen.apply(_this3, args);
  ...
  _createClass(Engine, [{
     ...
     */
    get: function () {
      return this._rootPath || (this._rootPath = _basename$dirname$join.dirname(require.main.filename));
    },
