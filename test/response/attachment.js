...
test('res.attachment([filename]) when given a filename should set the filename param', async t => {
...
test('res.attachment([filename]) when omitting filename should not set filename param', async t => {
  const res = await response()
  res.attachment()
...
test('res.attachment([filename]) when given a no-ascii filename should work with http client', async t => {
  const app = new Trek()

  await app.initialize(false)

  app.middleware.use(({ res }) => {
